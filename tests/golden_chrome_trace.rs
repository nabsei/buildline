...
fn chrome_trace_of_ninja_fixture_matches_golden() {
    ...
    let got = common::normalize(&serde_json::to_string_pretty(&trace).unwrap());
    let expected = common::normalize(include_str!(
        "fixtures/chrome_trace/ninja_simple.expected.json"
    ));
